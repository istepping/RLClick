...
"""
神经网络训练过程的可视化包
torch.utils.tensorboard:tensorflow的TensorboardX在pyTorch中使用
"""


# 调用方法即可配置结果.
class Logger:
    def __init__(self, directory, write=True, save_freq=10):
        self.parent_dir = directory + r"/" + time.strftime("%Y%m%d-%H%M%S", time.localtime())
        self.write = write
        self.dir = ""
        self.fig_index = 0
        self.model_index = 0
        ...
        if self.write:
            self.boardWriter = SummaryWriter(self.parent_dir)
    ...
    def set_write(self, write=False):
        logger.write = write


logger = Logger(directory=r"../../exp/logs", write=False)
